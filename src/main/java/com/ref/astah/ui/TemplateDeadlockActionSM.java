...
public class TemplateDeadlockActionSM implements IPluginActionDelegate{
		...
		try {
			...
			if (diagram instanceof IStateMachineDiagram) {
				...
				new Thread(new Runnable() {
					...
					public void run() {
						try {
							StateMachineController.getInstance().AstahInvocation(diagram, VerificationType.DEADLOCK,progressBar);
